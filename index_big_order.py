...
STOCK_FNO_EXPIRY  = "2026-01-27"   # NSE stock options expiry (OPTSTK)
...
SUCCESS_LOCK = SUCCESS_FILE + ".lock"

# ================= FILE LOCK HELPERS =================

def acquire_file_lock(lock_path, timeout=10, poll_interval=0.05):
    """Simple lock using a .lock file so multiple processes don't race."""
    start = time.time()
    while True:
        try:
...
def load_existing_successes():
    """
    Load existing success records and fill success_keys using the same
    canonical key: underlying|strike|option_type|price.
    """
    global success_keys
    success_keys.clear()
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                if not underlying or pd.isna(strike) or not option_type or pd.isna(price):
                    continue
        ...
        print(f"📄 Loaded {len(success_keys)} existing success records from {SUCCESS_FILE}")
    except Exception as e:
        print(f"⚠️ Could not load success file: {e}")


def load_existing_entries():
    """
    Load existing entry records and fill entry_keys using the same
    canonical key: underlying|strike|option_type|price.
    'symbol' column in CSV is raw FO key (e.g. NSE_FO:BOSCHLTD25DEC37500PE),
    so we parse it to get underlying/strike/option_type.
    """
    global entry_keys
    entry_keys.clear()
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                price      = row.get("price")
                if not symbol_key or pd.isna(price):
                    continue
        ...
        print(f"📄 Loaded {len(entry_keys)} existing entry records from {ENTRY_FILE}")
    except Exception as e:
        print(f"⚠️ Could not load entry file: {e}")


def record_success(symbol, side, qty, price, vol_diff, oi_diff, ltp, moneyness, pcr, oi_same, oi_opposite):
    """
    Store a successful whale fill into CSV + in-memory dict.
    ...
    row = {
        "timestamp": ts,
        "instrument_key": symbol,
        "symbol": underlying,     # underlying name
        "strike": strike,
        "option_type": option_type,
        "side": side,
        "qty": qty,
    ...
    try:
        ...
        df = pd.DataFrame([row])
        df.to_csv(SUCCESS_FILE, mode="a", index=False, header=not file_exists)
    finally:
        release_file_lock(fd, SUCCESS_LOCK)
...
def record_entry(symbol, side, qty, price, ltp, moneyness, pcr, oi_same, oi_opposite):
    ...
    try:
        file_exists = os.path.exists(ENTRY_FILE)
        df = pd.DataFrame([row])
        df.to_csv(ENTRY_FILE, mode="a", index=False, header=not file_exists)
    except Exception as e:
