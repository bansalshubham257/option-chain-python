...
class StockAnalysisService:
    ...
    def process_stock_data_combined(self, nse_stocks, threshold=0.05):
        """
        Combined method to process both 52-week extremes and financials for a list of stocks.
        This reduces redundant API calls and improves efficiency.
        
        Args:
            nse_stocks: List of NSE stock symbols to process
            threshold: Threshold percentage for 52-week extremes detection
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            batch_52week_results = []
            batch_financial_results = []
            batch_failures = 0
            
            # Function to process a single stock with retries
            def process_stock_with_retry(symbol, retry_count=0):
                nonlocal consecutive_failures, rate_limit_delay
                ...
                try:
                    ...
                    try:
                        ...
                        if not hist.empty and len(hist) >= 5:  # Minimum data points
                            ...
                            # Get timestamps for highs and lows
                            high_date = hist["High"].idxmax()
                            low_date = hist["Low"].idxmin()
                            ...
                            is_near_high = pct_from_high <= threshold
                            is_near_low = pct_from_low <= threshold
                            
                            if is_near_high or is_near_low:
                                status = "near_high" if is_near_high else "near_low"
                                week52_data = {
                                    "symbol": symbol,
                                    "current_price": round(current, 2),
                                    "week52_high": round(high_52, 2),
                        ...
                        print(f"Error processing 52-week data for {symbol}: {e}")
                    ...
                    # Fetch quarterly financials - try multiple report types
                    financials_data = None
                    for data_type in ['quarterly_financials', 'quarterly_income_stmt', 'quarterly_balance_sheet']:
                        try:
                            data = getattr(stock, data_type, None)
                    ...
                    # Reset rate limiting parameters on success
                    consecutive_failures = 0
                    rate_limit_delay = max(1.0, rate_limit_delay * 0.9)
                    ...
                    if retry_count < 3:  # Maximum 3 retries
                        ...
                        time.sleep(retry_delay)
                        return process_stock_with_retry(symbol, retry_count + 1)
                    else:
                        print(f"Error processing {symbol} after 3 retries: {e}")
                        return {"symbol": symbol, "error": str(e)}
            
            # Process batch with parallel workers
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                future_to_symbol = {executor.submit(process_stock_with_retry, symbol): symbol for symbol in batch}
                ...
                for future in concurrent.futures.as_completed(future_to_symbol):
                    ...
                    try:
                        ...
                        if result:
                            # Process 52-week data
                            if result.get("week52_data"):
                                batch_52week_results.append(result["week52_data"])
                            
                            # Process financials data
                            if result.get("financials"):
                                results['financials_stats']['success_count'] += 1
                                batch_financial_results.append({
                                    "symbol": result["symbol"],
                                    "financials": result["financials"],
                                    "next_earnings_date": result.get("next_earnings_date")
                                })
                            elif result.get("next_earnings_date"):
                                results['financials_stats']['skipped_count'] += 1
                                batch_financial_results.append({
                                    "symbol": result["symbol"],
                                    "financials": [],
                                    "next_earnings_date": result.get("next_earnings_date")
                                ...
                                if result.get("error"):
                                    batch_failures += 1
                                    results['financials_stats']['failed_count'] += 1
                        else:
                            batch_failures += 1
                            results['financials_stats']['failed_count'] += 1
            ...
            # Save financial results to database
            if batch_financial_results:
                self.database_service.save_quarterly_financials_batch(batch_financial_results)
            
            # Add 52-week results to overall results
            results['week52_data'].extend(batch_52week_results)
        ...
        results['completion_time'] = (datetime.now(ist) - now).total_seconds()

        # Save 52-week data to database
        if results['week52_data']:
            print(f"Saving {len(results['week52_data'])} 52-week results to database")
            self.database_service.save_52_week_data(results['week52_data'])
